...
class FilterPerformer(nn.Module):
    ...
    def restore(self, x, mask, arguments, apply_filter_mask=None):
        assert len(self.filters) == len(arguments)
        
        outputs = []
        _image = x
        if apply_filter_mask is None:
            apply_filter_mask = [True] * len(self.filters)
        for filter, arg, apply_filter_mask in zip(self.filters, arguments, apply_filter_mask):
            if apply_filter_mask:
                _image = filter(_image, arg)
                outputs.append(_image * mask + x * (1 - mask))
            else:
                outputs.append(_image)

        return outputs

    def adjust(self, image, mask, arguments, apply_filter_mask=None):
        assert len(self.filters) == len(arguments)
        ...
        if apply_filter_mask is None:
            apply_filter_mask = [True] * len(self.filters)
        for filter, arg, apply_filter_mask in zip(reversed(self.filters), reversed(arguments), reversed(apply_filter_mask)):
            if apply_filter_mask:
                _image = filter(_image, arg)
